...
export function getMiddleLogger (logger4js: any, options: any) {
  ...
  return async (ctx: any, next: Function) => {
    ...
    if (thislogger.isLevelEnabled(level) || options.level === "auto") {
      ...
      ctx.response.writeHead = function (code: any, headers: Function) {
      ...
      if (thislogger.isLevelEnabled(level)) {
        ...
        if (typeof fmt === "function") {
          const line = fmt(ctx, function (str: string) {
 ...
 * @return {Array}
 */
function assembleTokens (ctx: any, customTokens: any) {
  const arrayUniqueTokens = function (array: Array<any>) {
  ...
  customTokens = customTokens.map(function (token: any) {
 ...
 *         SAME AS "\\.jpg|\\.png|\\.gif"
 */
function createNoLogCondition (nolog: RegExp) {
  // tslint:disable-next-line:no-null-keyword
  let regexp = null;
  if (nolog) {
